...
module Router =
    ...
    let nav xs (mode: HistoryMode) (routeMode: RouteMode) =
        ...
        then history.pushState (obj(), "", encodeParts xs routeMode)
        else history.replaceState(obj(), "", encodeParts xs routeMode)
...
module ReactExtension =
    type React with
        ...
        static member inline router (props: IRouterProperty list) =
            let input = unbox<Router.RouterProps> (createObj !!props)
